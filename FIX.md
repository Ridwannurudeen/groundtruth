...
**Confirmed state (verified 2026-07-04):** 25 `retracted_original` + 15 `active_control` in `seed_corpus.json`; `held_back_retractions` already lists all **25** — the data to forget all of them is present. `claims.json` shows only **13** `retracted_forgotten`; these 12 remain `active` and in `groundtruth_memory`: **R013, R015, R016, R017, R018, R019, R020, R021, R022, R023, R024, R025**. **Root cause:** the benchmark run exhausted the Gemini 2.5-flash free-tier quota mid-pass (fell back to flash-lite, then stopped at 13). So the watcher/prep MUST be **resumable and idempotent** — re-running continues from where quota cut it off, never re-forgets or duplicates, and completes all 25 across quota windows. Verify this explicitly.
