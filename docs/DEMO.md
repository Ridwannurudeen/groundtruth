...
7. On the GroundTruth panel, click `Downvote`, then `Improve`. The feedback loop applies weights through Cognee's improve path and re-asks with `feedback_influence=1`.

## Presenter Notes

- Benchmark headline: naive memory cites retracted sources in 17/20 answers; GroundTruth cites them in 0/20.
- Control retention: 5/5 active controls remain available.
- Correctness judge is intentionally skipped because the Gemini free-tier quota is exhausted; the primary metric is deterministic citation status.
